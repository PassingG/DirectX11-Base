...
bool SystemClass::Initialize()
{
	...
	// m_Graphics 객체 생성. 그래픽 렌더링을 처리하기 위한 객체입니다.
...
void SystemClass::Run()
{
	...
	// 사용자로부터 종료 메시지를 받을때까지 메시지루프를 돈다.
	while (true)
	{
		// 윈도우 메시지를 처리한다.
...
void SystemClass::InitializeWindows(int& screenWidth, int& screenHeight)
{
	...
	if (FULL_SCREEN)
	{
		// 풀스크린 모드로 지정했다면 모니터 화면 해상도를 데스크톱 해상도로 지정하고 색상을 32bit로 지정한다.
